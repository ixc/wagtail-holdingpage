...
class HoldingPageTestMixin:
    def setUp(self):
        ...
        self.root_page = wagtail_factories.PageFactory.create(title="Root")
        self.home_page = wagtail_factories.PageFactory(
            parent=self.root_page, title="Homepage"
        )
        ...
        # Site/page ids are reused across tests, so stale cached root paths
        # from a previous test can resolve URLs against the wrong site.
...
class SettingsTest(HoldingPageTestMixin, TestCase):
    admin_path = "/admin/"
    ...
    def get_edit_instance_as_admin(self, instance, admin=None):
        """Load the edit page for a settings instance as an admin user."""
        path = self.get_edit_instance_path(instance)
        if admin is not None:
            self.client.force_login(admin)
        return self.client.get(path)

    def assert_page_loading(self, path, status_code=HTTPStatus.OK):
    ...
    def test_settings__can_edit(self):
        ...
        response = self.get_edit_instance_as_admin(
    ...
    def test_settings__can_allow_pages_admin(self):
        ...
        self.assertContains(response, "allowed_pages-TOTAL_FORMS")
    ...
    def test_settings__allowed_pages_are_accessible(self, include_descendants):
        settings = HoldingPageSettingsFactory(site=self.site, holdingpage_active=True)
        allowed_child = wagtail_factories.PageFactory(parent=self.home_page)
        allowed_grandchild = wagtail_factories.PageFactory(parent=allowed_child)
        HoldingPageAllowedPageFactory(
            settings=settings,
            page=allowed_child,
            include_descendants=include_descendants,
